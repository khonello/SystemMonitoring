...
- **`LabClient` is DONE.** Stock Win11 24H2 Pro, local account `lab`/`lab`, Python 3.12 from python.org installed **for all users** at `C:\Python312`, repo copied (not cloned) to `C:\SystemMonitoring`, venv at `.venv`, packages installed, `client --check` clean — id `labclient-1-windows`, three MISSING bundles, `agent running False` — static `192.168.100.3/24` with no gateway, Dynamic Memory 2/1/3 GB, automatic checkpoints off, and the **`baseline` checkpoint taken**.
- **Next: the Engine VM, and it is the only thing left before the gate.** Install Debian (**12 or 13, never 11** — bullseye ships Python 3.9, under the floor) with everything deselected in tasksel, attach `LabRepo.iso` to its DVD drive, then inside the guest: `mount -o ro /dev/sr0 /mnt && sh /mnt/scripts/lab_engine_setup.sh`. Then `lab_host_finalize.ps1 -VMName LabEngine` on the host, then the T0.1/T0.2 gate, then Part 1.
...
- `LabRepo.iso` is the repo-on-a-disc built with `oscdimg` (LAB-SETUP.md step 4). It is how code reaches both guests without ssh, shares or a network, and the same image serves the Engine VM.
