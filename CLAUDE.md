...
Phases 0–4 are complete: Engine, Administrator and Client Agent are all implemented and tested (289 tests). **Phase 5, manual per-system verification, is in progress — and it is in Part 0 (setup), not the tests.** `scripts/setup_lab_vms.ps1` has been run: Hyper-V storage is on `K:` (its `C:` default cannot hold a guest), the `LabMonitor` Internal switch exists, and both VMs are created. Remaining: install Windows in `LabClient` and Debian in `LabEngine`, provision both, then the T0.1/T0.2 gate. `testing.md` **section 0.5 is Part 0 as a flat checklist** — start there.
