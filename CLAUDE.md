...
Phases 0–4 are complete: Engine, Administrator and Client Agent are all implemented and tested (289 tests). **Phase 5, manual per-system verification, is in progress — and it is stalled in Part 0 (setup), not in the tests.** The client VM's trimmed install media is built (`scripts/build_client_image.ps1` → `K:\win11-labclient.iso`, rationale in `testing.md` section 0.1e); **no VM exists yet**. Remaining before any test runs: point Hyper-V's storage at `K:` (its `C:` default cannot hold a guest), create the `LabMonitor` Internal switch, build the Windows client VM, run the section 0.1e gate checks, then build the **Engine VM** — a second, separate Linux VM that is easy to forget because all the setup effort so far went into the Windows one. Packaging was moved out of Phase 4 to Phase 8 (dead last) — packaging code that has never been proven on its target machine is the wrong order of work, and no design decisions remain in it. Three code paths fall back to the running interpreter meanwhile and log a warning; those fallbacks are for bringing a machine up, not for shipping, so Phase 5's results do not fully transfer to the packaged build.
