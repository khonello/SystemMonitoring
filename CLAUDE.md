...
Phases 0–4 are complete: Engine, Administrator and Client Agent are all implemented and tested (289 tests). **Phase 5, manual per-system verification, is in progress. Both VMs are now built (2026-08-13) and Part 0 is done bar the T0.1/T0.2 gate** — the tests themselves have not started, and the results table in `testing.md` is still empty. Follow **`LAB-SETUP.md`**; it is the procedure, and its pitfalls table is symptom-first.
...
- **Next: the T0.1/T0.2 gate, then Part 1.** Start both VMs; on the Engine `cd /opt/SystemMonitoring && ENGINE_LOG_LEVEL=DEBUG python3 -m engine`, and from LabClient `Test-NetConnection 192.168.100.2 -Port 5000` must return `TcpTestSucceeded : True`. Nothing in Part 1 starts until it does.
